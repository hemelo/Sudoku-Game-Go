...
type ClientCLI struct {
	...
	currentGame       *internals.Game
	defaultDifficulty internals.Difficulty
}

func NewClientCLI(client internals.Client) *ClientCLI {
	return &ClientCLI{
		Client:        client,
		currentGame:   nil,
		currentGameID: "",
...
func (c *ClientCLI) handleCreateGame() {
	...
	game, gameID := c.GameController.CreateGame(difficulty)
...
func (c *ClientCLI) handleSelectGame() {
	...
	game, err := c.GameController.GetGame(gameID)
...
func (c *ClientCLI) handleGame() {
	...
	for {
		...
		case MakeMoveAction:
			...
			if err := c.GameController.MakeMove(c.currentGame, row, col, value); err != nil {
				fmt.Printf("Error: %v\n\n", err)
			} else {
				c.ClearScreen()
				c.printCurrentGame()
		...
		case RemoveMoveAction:
			...
			if err := c.GameController.RemoveMove(c.currentGame, row, col); err != nil {
				fmt.Printf("Error: %v\n\n", err)
			} else {
				c.ClearScreen()
				c.printCurrentGame()
		...
		case GetHintAction:
			hint, err := c.GameController.GetHint(c.currentGame)
			if err != nil {
				fmt.Printf("Error: %v\n\n", err)
			} else {
		...
		case SolveAction:
			if err := c.GameController.SolveGame(c.currentGame); err != nil {
				fmt.Printf("Error: %v\n\n", err)
			} else {
...
func (c *ClientCLI) printBoard(board internals.Board) {
...
func (c *ClientCLI) printGame(game internals.Game) {
...
func (c *ClientCLI) printGameHistory(game internals.Game) {

	boardTimeline, err := c.GameController.GetBoardHistory(&game)
	...
	for _, boardHistory := range boardTimeline {
		...
		if boardHistory.ActionType == internals.BoardRemovedHistoryAction {
		...
		} else if boardHistory.ActionType == internals.BoardAddedHistoryAction {
		...
		} else if boardHistory.ActionType == internals.BoardCreatedHistoryAction {
...
func (c *ClientCLI) promptForDifficulty() internals.Difficulty {
	...
	difficulty, err := internals.StringToDifficulty(result)
